...
(function () {
  ...
  function phaseHint(b) {
    ...
    if (b.phase === "combat")
      return UI.combatReport
        ...
        : "Select an engagement on the field, then order the fight and roll the dice.";
  ...
  UI.updateHud = function () {
    ...
    $("phase-track").innerHTML = phases
      ...
      .join("");
    if (b.phase === "combat") {
      $("battle-who").textContent = b.pendingRoll ? "Roll the dice" : UI.combatAnimating ? "Fighting" : "Choose the next engagement";
  ...
  UI.canvasClick = function (p, e) {
    var b = UI.battle;
    if (!b) return;
    if (UI.combatAnimating && !b.pendingRoll) return;
    var before = UI.sel;
  ...
  UI.renderCombatOrders = function () {
    ...
    act.innerHTML = "";
    panel.hidden = false;
    if (!b.pendingRoll && UI.diceHide) UI.diceHide();
    if (UI.combatReport) {
    ...
    if (UI.combatAnimating) {
      if (b.pendingRoll) {
        ...
        act.appendChild(elem("div", "combat-instruction", "<b>" + esc(sp.label) + "</b><br>" + esc(sp.sub)));
        act.appendChild(button("Roll the dice (Space)", UI.rollDice, "primary"));
        act.appendChild(button("Auto-resolve this engagement", UI.autoResolve));
        return;
      }
      panel.innerHTML =
        '<div class="engagement-kicker">Engagement resolving</div><h3>The lines collide</h3><p class="combat-instruction">Reading the dice…</p>';
      return;
    }
  ...
  UI.fightEngagement = function () {
    ...
    UI.sound("combat");
    // The dice panel now asks for each roll; the engagementResolved event finishes the fight.
    UI.processEvents();
    UI.updateHud();
  };
  UI.finishEngagement = function (report) {
    var b = UI.battle;
    if (UI.combatTimer) clearTimeout(UI.combatTimer);
    UI.combatTimer = setTimeout(
      function () {
        ...
        UI.combatAnimating = false;
        UI.combatReport = report;
        UI.combatChoice = null;
        UI.targets = [];
        UI.lastHint = null;
        if (UI.diceHide) UI.diceHide();
        if (!report) UI.continueCombat();
        else UI.updateHud();
      },
      UI.settings.motion ? 500 : 0,
    );
  };
  ...
  window.addEventListener("keydown", function (e) {
    ...
    if (UI.battle.pendingRoll) return; // Space/Enter roll the dice (handled by the dice panel)
